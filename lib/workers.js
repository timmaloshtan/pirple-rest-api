...
const alertUserToStatusChange = async checkData => {
  ...
  try {
    ...
    console.log('Success. User was alerted to the status change in their check via SMS');
  } catch (error) {
    console.error('Could not send an alert to user who had a status change in their check');
...
const processCheckOutcome = async (checkData, checkOutcome) => {
  ...
  if (!isAlertWarranted) {
    return console.info('Check outcome has not changed, no alert needed');
  }
...
const rotateLogs = async () => {
  ...
  if (logNames.length === 0) {
    return console.error('Could not find any logs to rotate');
  }
  ...
  logNames.forEach(async logName => {
    ...
    try {
      ...
      console.log('Successfully compressed and trunkcated the log');
    } catch (error) {
