...
handlers._users.post = async (data, callback) => {
  ...
  const tosAgreement = validation.checkTosAgreement(data.payload.tosAgreement);
...
handlers._users.get = async (data, callback) => {
  ...
  var phone = validation.checkPhone(data.queryString.phone);
  ...
  var token = validation.checkToken(data.headers.token);
