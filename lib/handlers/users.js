...
_users.delete = async data => {
  ...
  // Grab user checks from user data
  ...
  const deletionResults = await Promise.all(
    userChecks.map(
      id => _data.delete('checks', id)
        .then(() => true)
        .catch(() => false)
    )
  );

  if (deletionResults.every(deletion => deletion)) {
    return {
      statusCode: 200,
    };
  }
  ...
  return {
    statusCode: 500,
    payload: { error: 'Encountered errors while deleting checks. All checks may not have been deleted.'}
  };
};
