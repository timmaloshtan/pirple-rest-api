...
const promisifiedDnsResolve = util.promisify(dns.resolve);
...
_checks.post = async data => {
  ...
  const hostName = validation.checkNonEmptyString(parsedUrl.hostname);

  try {
    const records = await promisifiedDnsResolve(hostName);
  } catch (error) {
    return {
      statusCode: 400,
      payload: { error: 'The hostname of the URL entered did not resolve to DNS entry'},
    }
  }
